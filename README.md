...
- **流れ**：① 主催者が作問・保存 → ② 共通リンク（QR）で代表者が参加し一覧から
  ...
  進行（締切は手動）→ ④ 最後に総合順位を発表
- **識別**：匿名ログイン（個人情報なし）。主催者は枠を解放して交代も可能
...
### 4. 匿名認証を有効化
左メニュー「Authentication」→「ログイン方法」→「匿名」を有効化。
...
- 当日前に **テストモードを卒業**し、同梱の `firestore.rules`（認証済みのみ許可）を
  デプロイしてください：`npx firebase deploy --only firestore:rules`
